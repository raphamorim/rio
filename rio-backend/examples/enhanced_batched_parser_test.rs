...
fn main() {
    ...
    println!("   Default batch threshold: {} bytes", parser.batch_threshold());
    ...
    println!("   Buffer length after large input: {} bytes", parser.buffer_len());
    parser.flush(&mut performer);
    println!("   Buffer length after flush: {} bytes", parser.buffer_len());
