...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    GetCursorPos, GetMenu, GetMessageW, KillTimer, LoadCursorW, PeekMessageW,
    PostMessageW, RegisterClassExW, RegisterWindowMessageA, SetCursor, SetTimer,
    SetWindowPos, TranslateMessage, CREATESTRUCTW, GIDC_ARRIVAL, GIDC_REMOVAL, GWL_STYLE,
    GWL_USERDATA, HTCAPTION, HTCLIENT, MINMAXINFO, MNC_CLOSE, MSG, NCCALCSIZE_PARAMS,
    PM_REMOVE, PT_PEN, PT_TOUCH, RI_MOUSE_HWHEEL, RI_MOUSE_WHEEL, SC_MINIMIZE,
    ...
    fn wait_and_dispatch_message(&mut self, timeout: Option<Duration>) {
        fn get_msg_with_timeout(msg: &mut MSG, timeout: Option<Duration>) -> PumpStatus {
            unsafe {
                ...
                let timer_id =
                    timeout.map(|timeout| SetTimer(0, 0, dur2timeout(timeout), None));
