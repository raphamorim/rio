...
use ttf_parser::colr::{
    ClipBox, CompositeMode, GradientExtend,
    LinearGradient as TtfLinear, Paint, Painter,
    RadialGradient as TtfRadial,
};
use ttf_parser::{GlyphId, RgbaColor, Transform as TtfTransform};

use tiny_skia::{
    BlendMode, Color, FillRule, GradientStop, LinearGradient, Mask,
    Paint as SkPaint, Path, PathBuilder, Pixmap, PixmapPaint, Point,
    RadialGradient, Rect, Shader, SpreadMode, Transform,
};

use crate::font::glyf_decode;

/// Output of [`rasterize`]: an RGBA8 bitmap with the placement
/// metadata the atlas layer expects (mirrors the zeno monochrome
/// path in `glyph.rs`).
pub(super) struct RasterizedColor {
    pub data: Vec<u8>,
    pub width: u16,
    pub height: u16,
    ...
    fn paint(&mut self, paint: Paint<'a>) {
        match paint {
            Paint::Solid(color) => {
                ...
                p.shader = Shader::SolidColor(rgba_to_color(color));
                p.anti_alias = true;
                self.fill_current(p);
            }
            ...
            Paint::SweepGradient(sg) => {
                ...
                if let Some(first) = sg.stops(0, &[]).next() {
                    let mut p = SkPaint::default();
                    p.shader = Shader::SolidColor(rgba_to_color(first.color));
                    p.anti_alias = true;
                    self.fill_current(p);
                }
            }
...
fn first_base_glyph_id(colr: &[u8], glyphs: &[Vec<u8>]) -> Option<u16> {
    ...
    let is_non_empty = |gid: u16| -> bool {
        glyphs.get(gid as usize).is_some_and(|g| !g.is_empty())
    };
...
/// tiny-skia Pixmap pixels are premultiplied; the colour atlas wants
/// straight RGBA8. Unmultiply, clamping to `[0, 255]`.
fn pixmap_to_rgba(pixmap: &Pixmap) -> Vec<u8> {
    ...
    for p in pixels {
        ...
        let un = |c: u8| -> u8 {
            ((c as f32 * inv).round() as i32).clamp(0, 255) as u8
        };
        out.push(un(p.red()));
        out.push(un(p.green()));
        out.push(un(p.blue()));
...
fn stops_to_tiny_skia(stops: &[(f32, Color)]) -> Vec<GradientStop> {
    stops.iter().map(|&(o, c)| GradientStop::new(o, c)).collect()
}
...
mod tests {
    ...
    fn project_p3_rejects_degenerate_axis() {
        ...
        assert!(
            project_p3((10.0, 20.0), (50.0, 50.0), (10.0 + 1e-4, 20.0 + 1e-4))
                .is_none()
        );
    }
    ...
    fn first_base_glyph_id_honours_record_order() {
        ...
        let glyphs: Vec<Vec<u8>> =
            vec![vec![1]; 10]; // GID 0..9 all non-empty
    ...
    fn composite_mode_to_blend_covers_every_variant() {
        ...
        use CompositeMode::*;
        assert_eq!(
            composite_mode_to_blend(SourceOver),
    ...
    fn extend_to_spread_maps_all_three_modes() {
        ...
        assert_eq!(
            extend_to_spread(GradientExtend::Repeat),
            SpreadMode::Repeat
        );
        assert_eq!(
