...
// Unified shader system where all rendering uses the same vertex structure

/// Unified vertex structure used by all rendering components
#[repr(C)]
...
/// Get the unified shader source for the current context
pub fn get_unified_shader(supports_f16: bool) -> &'static str {
    if supports_f16 {
        include_str!("unified_f16.wgsl")
    } else {
        include_str!("unified.wgsl")
    }
}
