...
// Helper function to detect if we're using Metal backend
fn is_metal_backend(context: &Context) -> bool {
    // Check if the adapter backend is Metal
...
impl RichTextBrush {
    pub fn new(context: &Context) -> Self {
        ...
        let use_f16 = context.supports_f16() && !is_metal_backend(context);
