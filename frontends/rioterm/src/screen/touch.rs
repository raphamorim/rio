...
use winit::event::{
    ElementState, Event, Ime, MouseButton, MouseScrollDelta, StartCause, Touch, TouchPhase,
    WindowEvent,
};

use crate::crosswords::pos::{Pos, Line, Column};
...
fn on_touch_motion(route: &mut Route, touch: Touch) {
    ...
    match touch_purpose {
        ...
        TouchPurpose::Zoom(zoom) => {
            ...
            if font_delta >= 0. {
                route.window.screen.change_font_size(FontSizeAction::Increase);
            } else {
                route.window.screen.change_font_size(FontSizeAction::Decrease);
            }
            println!("zoom motion: {}", font_delta);
        },
        TouchPurpose::Scroll(last_touch) => {
            ...
            *touch_purpose = TouchPurpose::Scroll(touch);
            ...
            println!("scroll motion: {}", delta_y);
        },
        TouchPurpose::Select(_) => {
            let x = touch.location.x.clamp(
                0.0,
                route.window.screen.sugarloaf.layout.width.into(),
            ) as usize;
            let y = touch.location.y.clamp(
                0.0,
                route.window.screen.sugarloaf.layout.height.into(),
            ) as usize;
            route.window.screen.mouse.x = x;
            route.window.screen.mouse.y = y;
            println!("select motion");
        },
