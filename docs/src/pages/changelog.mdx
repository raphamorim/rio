...
- **Custom Cursor**: Configurable cursor effects via the new `effects` configuration
- **Trail Cursor**: Fading trail effect when the cursor moves (enabled by default via `effects.trail-cursor`)
