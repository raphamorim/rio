...
- Default URL/path regex now also matches local paths: rooted (`/abs`), explicitly-relative (`./x`, `../x`), home (`~/x`), env-var-rooted (`$VAR/x`), hidden-dir (`.config/x`), and bare relative paths with a dotted filename (`src/main.rs`). A post-match filter rejects mid-word `/` and `~/` matches (the `/bar` inside `foo/bar`) since Rust's regex crate has no lookbehind.
