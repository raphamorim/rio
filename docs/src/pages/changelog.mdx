...
- macOS Metal renderer: wide-gamut + gamma-correct color pipeline. `CAMetalLayer` now uses the `BGRA8Unorm_sRGB` pixel format (was `BGRA8Unorm`) and is tagged with the `kCGColorSpaceDisplayP3` colorspace. The `_sRGB` format makes Metal sRGB-encode on write and decode on read, so alpha blending runs in linear light — eliminates the "dark halo" / muddy-edge artifact around text and translucent overlays. The Display P3 colorspace widens the gamut ~26% past sRGB primaries, so configured theme colors land closer to Apple Terminal / ghostty's saturation. `renderer.metal` and `image.metal` now apply `srgb_to_linear` to fragment RGB before returning (alpha untouched) so the HW encode doesn't double-up on already-encoded theme colors; `MTLClearColor` goes through the same linearization on the Rust side (`sugarloaf::srgb_to_linear_f64`) so the window background doesn't gain a tint from the new pipeline. The three pipeline color attachments in `renderer/mod.rs` moved to `BGRA8Unorm_sRGB` to match the drawable. The `metal` crate doesn't wrap `CAMetalLayer.colorspace` yet, so the setter goes through `msg_send!`; the `CGColorSpace` handle is `mem::forget`-ed to guarantee the pointer stays live for the layer's lifetime, and a post-set read-back logs a warning if the colorspace didn't take.
