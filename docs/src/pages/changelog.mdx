...
- Fix kitty graphics protocol Unicode-placeholder mode (`kitten icat --unicode-placeholder`). Four bugs in series, all of which had to land for the image to actually render: (1) the APC parser stored `U=1` into a private `cmd.virtual_placement` field that was never propagated to `PlacementRequest`, so the routing in `place_graphic` always fell through to the direct-overlay branch; (2) `place_virtual_graphic` was auto-writing U+10EEEE cells to the grid, racing kitty's own writes (per the spec the application emits the cells, the terminal only stores metadata); (3) icat actually uses `a=T,U=1` (combined transmit-and-display), which goes through `kitty_transmit_and_display`, not `place_graphic` — that handler now also routes virtual placements correctly and pushes pixel data to `pending_images` (the regular overlay path did this implicitly via `place_kitty_overlay`); (4) the diacritics table in `kitty_virtual.rs` had 299 entries instead of the canonical 297, shifting every index past the divergence point so `image_id_high` decoded as the wrong byte (e.g. `0x5C` stored, `0x5E` decoded). Replaced with the exact 297 entries from `kitty/gen/rowcolumn-diacritics.txt`. Renderer side: new `Renderer::push_virtual_placeholder_overlays` scans visible rows once per frame for U+10EEEE cells, decodes `(image_id, placement_id, row_idx, col_idx)` from foreground color + combining diacritics (`kitty_virtual::decode_cell`), looks up the placement metadata, and pushes one `GraphicOverlay` per placement with aspect-preserving fit + centering — mirrors ghostty's `Placement.renderPlacement` (`graphics_unicode.zig:156-190`). Added `source_rect: [f32; 4]` to `GraphicOverlay` (default `[0,0,1,1]` via `FULL_SOURCE_RECT`) since the underlying `ImageInstance` already supported sub-rects but the overlay path was hardcoding the full image. Placeholder cells render with `style.background_color = None` so the per-cell bg quad doesn't cover the BelowText image overlay underneath. Tests cover the diacritic conversion table, `decode_cell` across all four input shapes (Indexed fg, Spec fg + 3rd diacritic, underline=placement_id, missing diacritics edge case), parser-level `U=1` propagation, the metadata-only contract of `place_virtual_graphic`, and an end-to-end pass that feeds icat's exact wire format and asserts the resulting grid cells carry the right codepoint + RGB fg + 3 diacritics.
