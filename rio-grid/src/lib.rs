...
pub use preedit::{PreeditCell, PreeditOverlay};
...
pub fn build_row_bg<P: GridPalette>(
    ...
    let preedit_block_bg = preedit
        .as_ref()
        .map(|_| normalized_to_u8(palette.named_colors().cursor));
...
pub fn build_row_fg<P: GridPalette>(
    ...
    if let Some(pre) = preedit {
        let preedit_text_fg = normalized_to_u8(palette.named_colors().background.0);
        for col in 0..cols {
        ...
        if let Some(caret_col) = pre.caret_col() {
            if caret_col < cols {
                emit_ime_caret(
                    ...
                    cell_h_u32,
                    thickness,
                    normalized_to_u8(palette.named_colors().cursor),
                    fg_scratch,
                );
            }
        }
...
fn emit_preedit_char(
    ...
    // Single-cell run: stamp the same key shape the main run loop
    // builds — `(codepoint, cluster=0)` followed by the
    // `(cell_count, font_id, size_bucket)` tail — so a composition
    // char shares the run cache with ordinary text.
    rasterizer.run_hasher = rapidhash::fast::RapidHasher::default();
