//! IME preedit overlay.
//!
//! Plotting model. The frontend hands us the in-flight composition
//! string and an optional IME cursor byte offset. We map that
...
impl PreeditOverlay {
    /// `text` is the in-flight composition string and
    /// `cursor_byte_offset` the IME cursor's byte offset into it (the
    /// two fields of the frontend's `Preedit`). Taking them apart
    /// keeps this crate free of any frontend IME type.
    pub fn new(
        text: &str,
        ...
        if text.is_empty() || columns == 0 || rows == 0 {
            return None;
        }
        ...
        if cursor_byte_offset == Some(text.len()) && ime_cursor_pos.is_none() {
            record_ime_cursor(row, col, &mut ime_cursor_pos);
        }
...
mod tests {
    ...
    fn places_wide_chars_and_spacers() {
        let overlay = PreeditOverlay::new("啊a", None, 0, 0, 4, 1).unwrap();
    ...
    #[test]
    fn wraps_wide_chars() {
        let overlay = PreeditOverlay::new("啊", None, 0, 2, 3, 2).unwrap();
    ...
    fn tracks_ime_cursor_at_end() {
        ...
        // composition.
        let text = "あい";
        let overlay = PreeditOverlay::new(text, Some(text.len()), 0, 0, 10, 1).unwrap();
    ...
    fn tracks_ime_cursor_inside_preedit() {
        ...
        let overlay = PreeditOverlay::new("あい", Some(3), 0, 0, 10, 1).unwrap();
    ...
    fn no_ime_cursor_when_offset_none() {
        let overlay = PreeditOverlay::new("hi", None, 0, 0, 10, 1).unwrap();
    ...
    fn ime_cursor_at_start_matches_terminal_cursor() {
        ...
        let overlay = PreeditOverlay::new("abc", Some(0), 0, 2, 10, 1).unwrap();
    ...
    fn empty_preedit_returns_none() {
        assert!(PreeditOverlay::new("", None, 0, 0, 10, 1).is_none());
    }
}
