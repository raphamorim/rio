...
// The grid/cell/geometry/style/attr primitives live at the canario crate root
// (`crate::{grid, pos, square, style, attr}`). Re-export them under
// `crate::crosswords::*` so the engine's siblings (`search`, `vi_mode`,
// `selection`) and rio-backend's re-export shim keep resolving the historical
// `crate::crosswords::{grid, pos, square, style, attr}` paths unchanged.
pub use crate::{attr, grid, pos, square, style};
pub mod search;
pub mod vi_mode;
...
use crate::host::{Alert, ClipboardKind, TerminalHost};
use crate::selection::{Selection, SelectionRange, SelectionType};
use crate::square::{CellFlags, Wide};
use crate::TermColors;
use crate::TerminalDamage;
use rio_core::color::{self as colors, ColorRgb};
...
use grid::row::Row;
use pos::{
    Boundary, CharsetIndex, Column, CursorState, Direction, Line, Pos, Side,
};
...
use std::ptr;
use std::sync::Arc;
use rio_core::{GraphicData, MAX_GRAPHIC_DIMENSIONS};
    ...
    H: TerminalHost,
{
    ...
    /// Codepoints this session has registered over Glyph Protocol
    /// (APC `25a1`). The render-side glossary itself lives in the host
    /// (the embedder owns the font types); the engine keeps only the
    /// set of registered codepoints so a `c` (clear) reply can be
    /// formed and a re-register is idempotent. Empty until a program in
    /// this session actually uses the protocol, so idle terminals pay
    /// nothing.
    glyph_codepoints: rustc_hash::FxHashSet<u32>,
    pub cursor_shape: CursorShape,
    pub default_cursor_shape: CursorShape,
    pub blinking_cursor: bool,
    pub window_id: H::WindowId,
    pub route_id: usize,
...
impl<H: TerminalHost> Crosswords<H> {
    pub fn new<D: Dimensions>(
        ...
        Crosswords {
            ...
            glyph_codepoints: rustc_hash::FxHashSet::default(),
    ...
    pub fn mark_fully_damaged(&mut self) {
        ...
        if !was_damaged {
            self.host.alert(self.window_id, Alert::Damaged);
        }
    }
    ...
    pub fn scroll_display(&mut self, scroll: Scroll) {
        ...
        self.host.alert(self.window_id, Alert::MouseCursorDirty);
    ...
    /// Read-only access to the embedder's [`TerminalHost`]. Used by tests and
    /// embedders to inspect host-side state (e.g. a glyph registry the host
    /// owns) without mutating the engine.
    #[inline]
    pub fn host(&self) -> &H {
        &self.host
    }
    ...
    pub fn send_graphics_updates(&mut self) {
        if self.graphics.has_pending_updates() {
            if let Some(queues) = self.graphics.take_queues() {
                self.host.update_graphics(self.window_id, queues);
            }
        }
    }
    ...
    where
        H: TerminalHost,
    {
        self.host.alert(self.window_id, Alert::ChildExited);
    }
        ...
        H: TerminalHost,
    {
        ...
        self.host
            .alert(self.window_id, Alert::CursorBlinkingChanged);
    }
    ...
    pub fn snapshot_visible(
        &mut self,
        damage: &crate::damage::TerminalDamage,
        cols: usize,
...
impl<H: TerminalHost> Handler for Crosswords<H> {
    #[inline]
    ...
    fn dynamic_color_sequence(&mut self, prefix: String, index: usize, terminator: &str) {
        ...
        // OSC 4/10/11 colour *query*: the host resolves the palette slot
        // asynchronously and applies this formatter to the resolved RGB.
        self.host.color_request(
            self.window_id,
            index,
            Arc::new(move |color| {
                format!(
    ...
    fn set_progress_report(&mut self, report: crate::host::ProgressReport) {
        self.host.alert(self.window_id, Alert::Progress(report));
    }
    ...
    fn clipboard_store(&mut self, clipboard: u8, base64: &[u8]) {
        let clipboard_kind = match clipboard {
            b'c' => ClipboardKind::Clipboard,
            b'p' | b's' => ClipboardKind::Selection,
            _ => return,
        };
    ...
    fn identify_terminal(&mut self, intermediate: Option<char>) {
        match intermediate {
            None => {
                ...
                self.host.write_pty(self.window_id, text.as_bytes());
            }
            Some('>') => {
    ...
    fn reset_color(&mut self, index: usize) {
        ...
        self.host.alert(
            self.window_id,
            Alert::PaletteChanged { index, color: None },
        );
    }

    #[inline]
    fn bell(&mut self) {
        self.host.alert(self.window_id, Alert::Bell);
    }

    #[inline]
    fn desktop_notification(&mut self, title: String, body: String) {
        self.host
            .alert(self.window_id, Alert::Notification { title, body });
    }
    ...
    fn clipboard_load(&mut self, clipboard: u8, terminator: &str) {
        ...
        // OSC 52 paste: the host reads the requested clipboard buffer
        // asynchronously and applies this formatter (base64 + OSC framing)
        // before writing the reply to the originating PTY.
        self.host.clipboard_load_request(
            self.window_id,
            clipboard_kind,
            Arc::new(move |text| {
    ...
    fn text_area_size_pixels(&mut self) {
        ...
        self.host.text_area_size_request(
            self.window_id,
            Arc::new(move |window_size| {
                let height = window_size.height_px;
                let width = window_size.width_px;
    ...
    fn graphics_attribute(&mut self, pi: u16, pa: u16) {
        ...
        self.host
            .write_pty(self.window_id, generate_response(pi, ps, pv).as_bytes());
    }
    ...
    fn kitty_graphics_response(&mut self, response: String) {
        ...
        self.host.write_pty(self.window_id, response.as_bytes());
    }
    ...
    fn glyph_register(
        ...
        // API, tests) still get the rejection before any host work.
        if !is_pua(cp) {
            return Err(RegisterError::OutOfNamespace);
        }

        // The host owns the render-side glossary (it owns the font
        // types). It validates the payload, stores it, and installs the
        // registry into its font library on first use. The engine only
        // remembers which codepoints were registered so an idle terminal
        // pays nothing and re-registers stay idempotent.
        let result = self.host.glyph_register(self.window_id, cp, payload);
        if result.is_ok() {
            self.glyph_codepoints.insert(cp);
        }
        result
    }
    ...
    fn glyph_clear(&mut self, cp: Option<u32>) {
        ...
        if self.glyph_codepoints.is_empty() {
            return;
        }
        match cp {
            None => self.glyph_codepoints.clear(),
            Some(cp) => {
                self.glyph_codepoints.remove(&cp);
            }
        }
        self.host.glyph_clear(self.window_id, cp);
    }

    fn glyph_query(&mut self, cp: u32) {
        // Defer to the host: only it has access to both the per-route
        // registry and the FontLibrary, so only it can compute the
        // System / Glossary / Both bits accurately. The host formats the
        // reply and writes it back to this pane's PTY asynchronously.
        self.host.glyph_query(self.window_id, cp);
    }
...
impl<H: TerminalHost> Dimensions for Crosswords<H> {
    #[inline]
    fn columns(&self) -> usize {
        self.grid.columns()
    }
...
impl<H: TerminalHost> Crosswords<H> {
    ...
    fn place_kitty_overlay(
        ...
        if placement.columns > 0 || placement.rows > 0 {
            ...
            graphic_data.resize = Some(rio_core::ResizeCommand {
                width: if placement.columns > 0 {
                    rio_core::ResizeParameter::Cells(placement.columns)
                } else {
                    rio_core::ResizeParameter::Auto
                },
                height: if placement.rows > 0 {
                    rio_core::ResizeParameter::Cells(placement.rows)
                } else {
                    rio_core::ResizeParameter::Auto
                },
...
mod tests {
    ...
    fn make_term_for_selection(rows: usize, cols: usize) -> Crosswords<VoidHost> {
    ...
    fn test_keyboard_mode_report() {
        ...
        // The report_keyboard_mode function writes a reply through the host.
        // We can't easily test the exact output without mocking the host,
    ...
    fn test_xtversion_report() {
        ...
        // A host that captures the bytes the engine writes back to the PTY.
        ...
        impl TerminalHost for CaptureHost {
            ...
            fn write_pty(&mut self, _id: Self::WindowId, bytes: &[u8]) {
        ...
        let mut term =
            Crosswords::new(size, CursorShape::Block, host, window_id, 0, 10_000);

        // Call report_version using Handler trait
        Handler::report_version(&mut term);

        // Verify that a single PTY write was emitted with the right format.
        let captured = writes.borrow();
        assert_eq!(captured.len(), 1, "Should have written one reply");

        let text = String::from_utf8(captured[0].clone()).expect("utf8 reply");
    ...
    fn sixel_stores_graphic_in_extras_table() {
        ...
        let graphic = GraphicData {
            id: rio_core::GraphicId::new(0), // will be reassigned
            width: 20,
            height: 20,
            pixels: vec![0u8; 20 * 20 * 4],
            color_type: rio_core::ColorType::Rgba,
            is_opaque: true,
            display_width: None,
            display_height: None,
            resize: None,
    ...
    fn new_term(cols: usize, rows: usize) -> Crosswords<VoidHost> {
        let size = CrosswordsSize::new(cols, rows);
        let window_id = ();
        Crosswords::new(
            size,
    ...
    fn icat_unicode_placeholder_wire_sequence_lands_in_grid() {
        ...
        let mut processor = crate::handler::Processor::default();
