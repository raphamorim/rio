...
//
// `Hyperlink` and the `ExtrasId` slot index are the self-contained cell-value
// helpers; they sit alongside the packed `Square(u64)` cell, its bit layout,
// and the `Extras` side-table record.

use crate::grid::row::Row;
use crate::grid::GridSquare;
use crate::pos::Column;
use crate::style::{StyleId, DEFAULT_STYLE_ID};
...
mod tests {
    ...
    fn bg_only_cells_are_not_empty() {
        let mut s = Square(0);
        s.set_bg_palette(7);
        assert!(!<Square as crate::grid::GridSquare>::is_empty(&s));
    }
