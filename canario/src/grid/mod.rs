...
//
// The `Dimensions` trait was originally taken from Alacritty
// https://github.com/alacritty/alacritty/blob/e35e5ad14fce8456afdd89f2b392b9924bb27471/alacritty_terminal/src/grid/mod.rs
// which is licensed under Apache 2.0 license.
...
use crate::pos::Pos;
use crate::pos::{Column, Cursor, Line};
use row::Row;
use std::cmp::{max, min};
...
pub struct Grid<T> {
    ...
    pub style_set: crate::style::StyleSet,
...
pub struct ExtrasTable {
    slots: Vec<Option<crate::square::Extras>>,
    free: Vec<u16>,
}
...
impl ExtrasTable {
    ...
    pub fn get(&self, id: crate::square::ExtrasId) -> Option<&crate::square::Extras> {
        self.slots.get(id as usize)?.as_ref()
    }

    pub fn get_mut(
        &mut self,
        id: crate::square::ExtrasId,
    ) -> Option<&mut crate::square::Extras> {
        self.slots.get_mut(id as usize)?.as_mut()
    }

    /// Allocate a new extras slot, returning its id (always non-zero).
    pub fn alloc(&mut self, extras: crate::square::Extras) -> crate::square::ExtrasId {
    ...
    pub fn free(&mut self, id: crate::square::ExtrasId) {
        if id == 0 {
            return;
        }
...
impl<T: GridSquare + Default + PartialEq + Clone> Grid<T> {
    pub fn new(lines: usize, columns: usize, max_scroll_limit: usize) -> Grid<T> {
        Grid {
            ...
            lines,
            columns,
            style_set: crate::style::StyleSet::new(),
...
impl Grid<Square> {
    ...
    #[inline]
    pub fn blank_with_bg(&mut self, bg: rio_core::color::AnsiColor) -> Square {
        use rio_core::color::{AnsiColor, NamedColor};
...
/// A `(screen_lines, columns)` tuple as a minimal `Dimensions` source.
///
/// This impl lived behind `#[cfg(test)]` in `rio-backend`. Because the trait
/// now lives here, the orphan rule prevents `rio-backend` from providing this
/// impl for the foreign tuple type, so its tests rely on `canario` exporting
/// it. It is provided unconditionally for that reason; it carries no runtime
/// cost and preserves the original test-only behavior.
