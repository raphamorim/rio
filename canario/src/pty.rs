...
//! engine.
//!
//! Lifted from `rio-backend`'s performer module behind the `pty` feature. It
//! owns the mio/`corcovado` poll loop, the channel `Msg` protocol, and the
//! read/write state machine; it feeds bytes through the [`Processor`]
//! (`handler::Processor`) into [`Crosswords`](crate::crosswords::Crosswords),
//! and nudges the host for repaints through the decoupled
//! [`TerminalHost`](crate::host::TerminalHost) — it names no `RioEvent`.

use crate::crosswords::Crosswords;
use crate::handler;
use crate::host::{Alert, TerminalHost};
...
/// Messages sent down the PTY channel to the [`Machine`] event loop.
#[derive(Debug)]
pub enum Msg {
...
pub struct Machine<T: teletypewriter::EventedPty, H: TerminalHost> {
    sender: channel::Sender<Msg>,
    receiver: PeekableReceiver<Msg>,
    pty: T,
    poll: corcovado::Poll,
    terminal: Arc<FairMutex<Crosswords<H>>>,
    host: H,
    window_id: H::WindowId,
    // Retained for constructor-signature compatibility with the frontend; the
    // route is now carried inside `window_id` (`H::WindowId`), which the host
    // uses to route the `Alert::Damaged` repaint nudges this driver emits.
    #[allow(dead_code)]
    route_id: usize,
}
...
impl<T, H> Machine<T, H>
where
    T: teletypewriter::EventedPty + Send + 'static,
    H: TerminalHost + Send + 'static,
{
    pub fn new(
        terminal: Arc<FairMutex<Crosswords<H>>>,
        pty: T,
        ...
        window_id: H::WindowId,
        route_id: usize,
    ) -> Result<Machine<T, H>, Box<dyn std::error::Error>> {
